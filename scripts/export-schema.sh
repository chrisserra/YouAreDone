...
# Remove AUTO_INCREMENT counters so schema diffs stay cleaner in git.
sed -E 's/AUTO_INCREMENT=[0-9]+ //g' "$TMP_FILE" > "$OUTPUT_FILE"
rm -f "$TMP_FILE"

echo "Exported schema to $OUTPUT_FILE"